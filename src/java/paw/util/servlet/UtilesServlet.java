...
public class UtilesServlet {
   ...
   * </ul>
   * 
   * <p>Este m&eacute;todo trata de construir un objeto-bean tomando 
   * directamente los valores para las propiedades del objeto de la QueryString de 
   * una petición (POST o GET). El m&eacute;todo requiere que los nombres de los 
   * propiedades del objeto coincidan con los nombres de los parámetros de la QueryString.
   *
   * <p>En el caso de que los nombres de los par&aacute;metros no coincidan con los
   * de los atributos del bean, es necesario usar otra versión del método que incluye
   * un argumento opcional <code>mappings</code> ({@link #populateBean(String,javax.servlet.http.HttpServletRequest,java.lang.String[][]) populateBean()}).
   *  
   * <p>El m&eacute;todo toma los par&aacute;metros de la QueryString y usa 
   * los valores de todos aquellos que tengan como nombre el nombre de alguna 
   * propiedad del bean para inicializar el bean. 
   * 
   * El m&eacute;todo hace las conversiones de tipos necesarias: <ul>
   ...
   * 
   * <p><b>Ejemplo 1. Todos los nombres de parámetros coinciden con los nombres de 
   * las propiedades del bean a construir</b>
...
<b>public</b>  <b>class</b> MiBean {
   ...
   * 
   * <p><b>Ejemplo 2. Cuando no coinciden todos los nombres de los parámetros y las 
   ...
   */
  public static Object populateBean(String className, HttpServletRequest request) throws ServletException {  
    try {
      Object bean = Class.forName(className).newInstance();
      BeanUtils.populate(bean, getBeanParams(request));
      return bean;
  ...
  public static Object populateBean(String className, HttpServletRequest request, String[]... mappings) throws ServletException {
    try {
  ...
  public static Object populateBean(Class theClass, HttpServletRequest request, String[]... mappings) throws ServletException {    
    try {
      Object bean = theClass.newInstance();
      Map params = getBeanParams(request);
      if (mappings != null) {
  ...
  private static Map getBeanParams(HttpServletRequest request) {
